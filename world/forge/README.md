...
R rounds (`R01`, …) run on the step machine (`engine/runner.ts`, steps in `engine/steps/`). This build registers steps 00-start … 11e-agreement: start, topic, brief, baseline, freeze, sealed forecasts, seal, probe mirror, writers and mechanical gate (00–05a), then defect copy, LLM gate, blind resubmission, decoy, champion and aux taste pairs, measures, unseal, surprise, aggregate and the two owner waits (05b–09b), then the merge (10a–10f) and bookkeeping (11a–11e). A round of this build ends `done` at 11e; the benchmark cycle (11f–11j), the wiki list, the bookkeeping commit and step 12 (11k–12b, defined in `bookkeeping.ts` / `final.ts`) are registered by the next slice.
...
- Exit codes: 0 done · 1 usage (config, a live engine lock, another round open, wrong branch with a dirty tree, refused redo, `forge merge` before 09b, post-merge drift under `--check`) · 2 waiting for the owner (`status.json` names what; also a merge rewind to 09b) · 3 integrity (marker, output or pinned-input drift, an owner file whose hash differs from its owner-log entry) · 4 blocked (probe mirror, public-content scan, quota, calibration, git / GitHub, a merge commit or push) · 5 failed (doctor red, void baseline or decoy, no passing submission after resubmission, missing precondition, a failed merge check after the canon was restored, fewer than two valid merge gate verdicts).
