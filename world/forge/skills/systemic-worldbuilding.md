<!--
Forge writer snapshot of the systemic-worldbuilding skill (inlined into writer prompts; SHA-256 pinned in freeze.json.skills).
Upstream: https://github.com/jwynia/agent-skills/tree/e02ec7e226a6e4f8419fd3b88a1d8e472d421b32/skills/creative/fiction/worldbuilding/systemic-worldbuilding
Revision: e02ec7e226a6e4f8419fd3b88a1d8e472d421b32 (license: MIT, declared in the upstream SKILL.md)
Local changes: guidance for existing canon and scoped output; output discovery follows the project's directories. Snapshot changes: frontmatter, the SOURCE.md pointer and the operator-specific project note removed.
-->
...
For an existing project, read its current canon, candidate status, and writing conventions first. Treat every new consequence as a hypothesis until the user adopts it. Examples such as FTL or alternate physics are prompts, not facts about the project. Respect a read-only request; write only when the task calls for a durable artifact. Follow the project's established directories and ask for an output location only when it cannot be inferred.
...
For a requested durable artifact, write the primary output to a file so work persists across sessions. For a read-only review, report findings in conversation.

### Output Discovery

1. Check the project's existing worldbuilding layout and instructions; use `context/output-config.md` when present.
2. If an authorized file output has no inferable location, ask the user where to save it.
3. Do not create a new output configuration or directory solely to use this skill.
...
### Session Persistence
- **Output location:** Follow project conventions, or `context/output-config.md` if present
