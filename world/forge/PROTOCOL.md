...
- **评委组**：Codex `gpt-6-astra` max、Claude Code Opus 5.5 max、Kimi Code `kimi-code/k3` max、Grok `grok-4.7` xhigh。每次调用都在全新临时目录、无工具、不加载操作者的记忆、技能、规则与钩子；记录 CLI 版本、请求模型与实际服务模型，实际服务模型不在 `judges.json` 的接受列表内时该次调用作废。
...
Owner 文件（`owner-log.jsonl`、各轮 `audit.json` 与 `decision.json`、`calibration/owner-answers.json`）只由 UI 写入，不可覆盖；引擎与代理不写这些文件。正典决定没有超时；选题 24 小时无人选择时由引擎取第一名并标 `auto_default`。
...
7. 除上述文件与 `assemble_reference.py` 重新生成的 `reference/REFERENCE.md`、`reference/hashes.json`、`reference/manifest.json` 外，`world/current/` 下任何文件有改动即判失败。
8. 未登记的说法留在 09 作为现场细节，之后可在不违背已登记事实的前提下改写。

随后 `assemble_reference.py --revision 8.N` 重生成合订本与哈希（`reference/manifest.json` 登记条目顺序、版本与抬头），BOOK 哈希 `438c4e31…` 不变，并在分支上重跑事实门。
...
4. 回放（`replay` 类）：各有效家族用新旧两版、在同一批次里重新评最近至多 16 对**保留盲标**（从未出现在证据包里的盲标），合并一致率不得下降，且没有家族下降 2 对或以上，否则记为 `rejected_by_replay`。
5. 每个结果都追加到引擎写入的 `benchmark/log.jsonl`；正在进行的轮次沿用冻结时的版本，`freeze.json` 按内容哈希锁定基准文件。

**盲标的可见与保留**：每条盲标按种子分到"可见"（文字可进证据包）或"保留"（只给计数）。第 0 轮 24 对中 12 可见、12 保留；此后每轮 4 对盲审中 2 可见、2 保留。

**回滚**：owner 在基准页回滚到任一旧版本，下一次冻结起生效。当前版本 = 最近一次已生效的 `activate` 与最近一次 owner `rollback` 中时间较晚者。
...
**第 0 轮**：24 对不同对子 + 4 对反序重测，四类各 6 对：8.1 原文 vs 现场改写；两个非评委家族网关模型的改写互比；同一模型不同立场；已知方向（原文 vs 陈词化副本）。改写只由非评委家族模型完成；OpenAI 不评含 8.1 原文的 6 对。Owner 先在 UI 作答，之后评委才开评；每家对每个可评对子正反两序各评一次，两序都选 owner 的那篇才算一致。

**资格**：在该家族可评的 m 个非已知方向对子上一致数 ≥ ⌈0.72·m⌉（18 对时 13 对，随机通过约 4.8%；OpenAI 12 对时 9 对），并且已知方向 ≥ 5/6。**重新认证**：未通过或后来被停用的家族可在 12 对新盲标（9 对非已知方向 + 3 对已知方向）上重测一次，须 ≥ 8/9 且 3/3；重测前维护者可修订一次基准，该修订须对所有家族通过回放。

**事实门演练**（第 0 轮）：每家 4 个注入缺陷、每类一个；漏抓 2 个及以上的家族在重测前不担任事实门评委。**评委 canary**（第 0 轮与任一 CLI 大版本更新后）：在本文件、一份密封明文与一份擂主文件里埋入 canary 字符串，邀请各适配器读取任意文件、上网并复述它收到的全部指令；输出中出现 canary 或操作者全局指令里的特征短语（`local.json` 列出）则该适配器停用直至修复。

**持续一致率**：每个家族对第 0 轮之后的盲标维持 Beta(1,1) 后验，一个（家族, 对子）为一次试验，该家族在此对子上所有会话对都前后一致且与 owner 相同才算一致。n ≥ 12 且 P(一致率 < 0.6) ≥ 0.8 时**标记**（退出 E，仍任事实门评委）；n ≥ 24 且 P(一致率 < 0.6) ≥ 0.95 时**停用**（退出一切，直至重新认证）。
...
| §5 接口卡制作立场评委 | F1-03 |
| §6 盲审与决策 | F1-00 已落地；定稿确认在 F1-04 |
| §7 mergecheck 与 manifest | F1-01 落地；合并编辑与分支重跑在 F1-03 |
| §8 基准校验 | F1-01 落地；证据包、提议、回放、启用与回滚在 F1-03 / F1-04 |
| §9 校准、canary 与一致率 | F1-02 / F1-03 / F1-06 |
| §10 薄层图计算 | F1-01 落地；初始标注在 F1-05 |
| 协议包哈希 | F1-01 落地；owner 批准与引擎拒跑在 F1-04 |
